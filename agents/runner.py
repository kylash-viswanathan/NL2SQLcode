...
from observability.langfuse_setup import get_callback_handler, get_langfuse_client
...
def _trace_context(thread_id: str, trace_name: str):
    """Sets a descriptive trace name + session_id (groups a HITL pause/resume pair
    into one Langfuse session) for this call. No-op context manager when Langfuse
    isn't configured, so callers don't need to branch on it."""
    if get_langfuse_client() is None:
        return contextlib.nullcontext()
    return propagate_attributes(trace_name=trace_name, session_id=thread_id)
...
def start_question(question: str, thread_id: str) -> dict:
    ...
    graph = get_graph()
    with _trace_context(thread_id, "answer-nl2sql-question"):
        return graph.invoke(
            {"question": question}, config={**_config(), "configurable": {"thread_id": thread_id}}
        )


def resume_with_decision(thread_id: str, approved: bool) -> dict:
    graph = get_graph()
    with _trace_context(thread_id, "resume-hitl-review"):
        return graph.invoke(
            Command(resume={"approved": approved}),
            config={**_config(), "configurable": {"thread_id": thread_id}},
        )
